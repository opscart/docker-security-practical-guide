...
echo ""
echo -e "1. ${BLUE}Default Isolation${NC}"
echo "   - Containers on different networks cannot communicate"
echo "   - This is a security feature, not a bug"
echo "   - Reduces blast radius if one container is compromised"
echo ""
echo -e "2. ${BLUE}DNS-Based Service Discovery${NC}"
echo "   - Containers on same network can find each other by name"
echo "   - No need to hardcode IP addresses"
echo "   - Automatic load balancing across replicas"
echo ""
echo -e "3. ${BLUE}Controlled Connectivity${NC}"
echo "   - Use 'docker network connect' for cross-network access"
echo "   - Only connect containers that truly need to communicate"
echo "   - Principle of least privilege"
echo ""
echo -e "4. ${BLUE}Production Best Practice${NC}"
echo "   - Create separate networks for each tier (web/app/db)"
echo "   - Only API gateways should span multiple networks"
echo "   - Never put everything on the default bridge network"
echo ""
echo -e "${YELLOW}Common Mistakes to Avoid:${NC}"
echo "- Using default bridge network (no DNS, less secure)"
echo "- Connecting all containers to all networks (defeats isolation)"
echo "- Using --network host (bypasses all isolation)"
echo ""
...
echo "✓ Created custom Docker networks"
echo "✓ Verified network isolation between containers"
echo "✓ Tested DNS-based service discovery"
echo "✓ Connected containers to multiple networks"
echo "✓ Understood security implications"
echo ""
echo -e "${BLUE}Network Architecture Created:${NC}"
echo ""
echo "frontend-net:"
echo "  - web-frontend (isolated)"
echo "  - api-frontend (isolated)"
echo "  - api-backend (gateway)"
echo ""
echo "backend-net:"
echo "  - web-backend (isolated)"
echo "  - api-backend (gateway)"
echo ""
echo -e "${BLUE}Next Steps:${NC}"
echo "- Run Scenario 2: Multi-Tier Network Segmentation"
echo "- See how to build production 3-tier architectures"
echo "- Learn internal networks for database security"
echo ""
